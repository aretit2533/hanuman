...
struct _http_client_async_handle_ {
    pthread_t thread;
...
static ssize_t socket_read(int sockfd, SSL *ssl, void *buf, size_t len)
{
    if (ssl) {
...
static char* parse_chunked_body(const char *chunked_data, size_t data_len, size_t *out_len)
{
    ...
    const char *end = chunked_data + data_len;
    ...
    while (ptr < end) {
        ...
        unsigned long chunk_size = strtoul(ptr, NULL, 16);
        ptr = line_end + 2;
        ...
        while (output_len + chunk_size > output_capacity) {
            output_capacity *= 2;
            output = (char*)realloc(output, output_capacity);
        }
        
        memcpy(output + output_len, ptr, chunk_size);
...
static char* decompress_gzip(const char *data, size_t data_len, size_t *out_len)
{
    z_stream stream;
    memset(&stream, 0, sizeof(stream));
    
    /* 15 + 32 for gzip decoding */
    if (inflateInit2(&stream, 15 + 32) != Z_OK) {
        return NULL;
    }
    
    size_t output_capacity = data_len * 4; /* Estimate 4x decompression */
    char *output = (char*)malloc(output_capacity);
    ...
    while ((ret = inflate(&stream, Z_NO_FLUSH)) == Z_OK) {
        if (stream.avail_out == 0) {
            ...
            size_t current_size = stream.total_out;
            output_capacity *= 2;
            output = (char*)realloc(output, output_capacity);
            stream.next_out = (Bytef*)(output + current_size);
            stream.avail_out = output_capacity - current_size;
        }
    }
...
static char* decompress_deflate(const char *data, size_t data_len, size_t *out_len)
{
    z_stream stream;
    memset(&stream, 0, sizeof(stream));
    
    /* Try raw deflate first (-15) */
    int init_result = inflateInit2(&stream, -15);
    if (init_result != Z_OK) {
        /* Try with zlib wrapper (default) */
        init_result = inflateInit(&stream);
        if (init_result != Z_OK) {
            /* Try with gzip wrapper (15 + 32) as fallback */
            init_result = inflateInit2(&stream, 15 + 32);
            if (init_result != Z_OK) {
                return NULL;
            }
        }
    ...
    size_t output_capacity = data_len * 4;
    char *output = (char*)malloc(output_capacity);
    ...
    if (body_len > 0) {
        ...
        for (size_t i = 0; i < response->header_count; i++) {
            if (strcasecmp(response->header_names[i], "Transfer-Encoding") == 0) {
                ...
                encoding = response->header_values[i];
            }
            if (strcasecmp(response->header_names[i], "Content-Encoding") == 0) {
                content_encoding = response->header_values[i];
            }
        }
        ...
        if (is_chunked) {
            decoded_body = parse_chunked_body(body_start, body_len, &decoded_len);
            if (!decoded_body) {
                response->error_message = strdup("Failed to parse chunked encoding");
                return response;
            }
        } else {
            decoded_body = (char*)malloc(body_len);
            memcpy(decoded_body, body_start, body_len);
        ...
        if (content_encoding || (encoding && !is_chunked)) {
            const char *enc = content_encoding ? content_encoding : encoding;
            
            if (strstr(enc, "gzip") != NULL) {
                final_body = decompress_gzip(decoded_body, decoded_len, &final_len);
                free(decoded_body);
                if (!final_body) {
                    response->error_message = strdup("Failed to decompress gzip");
                    return response;
                }
            } else if (strstr(enc, "deflate") != NULL) {
                final_body = decompress_deflate(decoded_body, decoded_len, &final_len);
                free(decoded_body);
                if (!final_body) {
                    response->error_message = strdup("Failed to decompress deflate");
                    return response;
                }
            } else {
        ...
        response->body = (char*)malloc(final_len + 1);
        if (response->body) {
            memcpy(response->body, final_body, final_len);
            response->body[final_len] = '\0';
            response->body_length = final_len;
        }
        
        if (final_body != decoded_body) {
            free(final_body);
        }
    }
...
HTTP_CLIENT_RESPONSE* http_client_execute(HTTP_CLIENT_REQUEST *request)
{
    ...
    /* Send request */
    if (socket_write(sockfd, ssl, request_buf, offset) < 0) {
        if (ssl) SSL_free(ssl);
        close(sockfd);
        HTTP_CLIENT_RESPONSE *response = (HTTP_CLIENT_RESPONSE*)calloc(1, sizeof(HTTP_CLIENT_RESPONSE));
...
{
    HTTP_CLIENT_ASYNC_HANDLE *handle = (HTTP_CLIENT_ASYNC_HANDLE*)arg;
    
    /* Execute the request */
    HTTP_CLIENT_RESPONSE *response = http_client_execute(handle->request);
    
    /* Update state */
    pthread_mutex_lock(&handle->mutex);
    if (handle->state != ASYNC_STATE_CANCELLED) {
        handle->state = ASYNC_STATE_COMPLETED;
        pthread_mutex_unlock(&handle->mutex);
        
        /* Invoke callback */
        if (handle->callback) {
            handle->callback(response, handle->user_data);
        } else {
            /* No callback, just free response */
    ...
    /* Free request */
    http_client_request_destroy(handle->request);
    handle->request = NULL;
    
    /* If detached, free the handle */
    pthread_mutex_lock(&handle->mutex);
    int is_detached = handle->detached;
    pthread_mutex_unlock(&handle->mutex);
                                                      ...
                                                      void *user_data)
{
    ...
    HTTP_CLIENT_ASYNC_HANDLE *handle = (HTTP_CLIENT_ASYNC_HANDLE*)calloc(1, sizeof(HTTP_CLIENT_ASYNC_HANDLE));
    if (!handle) {
        return NULL;
    }
    ...
    handle->state = ASYNC_STATE_RUNNING;
    handle->detached = 0;
...
int http_client_async_cancel(HTTP_CLIENT_ASYNC_HANDLE *handle)
{
    ...
    /* Mark as cancelled */
    pthread_mutex_lock(&handle->mutex);
    if (handle->state == ASYNC_STATE_RUNNING) {
        handle->state = ASYNC_STATE_CANCELLED;
    }
    int already_complete = (handle->state == ASYNC_STATE_COMPLETED);
    pthread_mutex_unlock(&handle->mutex);
    ...
    return already_complete ? FRAMEWORK_SUCCESS : FRAMEWORK_SUCCESS;
}
...
int http_client_async_is_complete(HTTP_CLIENT_ASYNC_HANDLE *handle)
{
    ...
    pthread_mutex_lock(&handle->mutex);
    int complete = (handle->state == ASYNC_STATE_COMPLETED);
    pthread_mutex_unlock(&handle->mutex);
